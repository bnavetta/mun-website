...
class SupplementalInfo extends React.PureComponent {
    ...
    async componentDidMount() {
        try {
            const [info, hotels] = await Promise.all([
                fetchSupplementalInfo(),
                fetchHotels(),
            ]);
            this.setState({
    ...
    renderForm(formApi) {
        return (
            <form
                ...
                className="supplemental-info-form form"
            >
    ...
    async handleSubmit(values, event, formApi) {
        try {
            ...
            if (result.success) {
                this.setState({
                    successMessage: "Supplemental information updated",
                    errorMessage: null,
                });
            } else {
                ...
                if (result.errors._global) {
                    this.setState({
                        successMessage: null,
                        errorMessage: result.errors._global.join(", "),
                    });
                } else {
                    this.setState({
                        successMessage: null,
                        errorMessage: "Some fields were invalid",
                    });
                }
            }
        } catch (error) {
    ...
    render() {
        ...
        if (loading) {
            ...
            return (
                <Form
                    defaultValues={supplementalInfo}
