    ...
    return (
        <div className="supplemental-info">
            <dl className="prop-list">
                ...
                <dd>{ `${info.streetAddress || ''}, ${info.city || ''} ${info.postalCode || ''}, ${info.country || ''}` }</dd>

                <dt>Hotel</dt>
                <dd>{ info.busunHotel ? info.busunHotel.name : info.nonBusunHotel }</dd>
...
class SupplementalInfoWrapper extends React.Component {
    ...
    loadSupplementalInfo() {
        ...
        fetchSupplementalInfo(this.props.id)
            .then(info => this.props.dispatch(loadSupplementalInfo(this.props.id, info)))
