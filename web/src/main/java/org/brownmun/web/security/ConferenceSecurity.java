...
public class ConferenceSecurity
{
    ...
    public static void authenticateAsAdvisor(Advisor advisor)
    {
        ...
        // This goes a bit more into Spring Security internals than I'd really like, but seems like a bit of a necessary
        // evil. Basically, it creates a new Authentication object backed by an AdvisorUser and stores that in the current
        // SecurityContext. Then, because there's a Spring Security filter to keep track of the current user in each
        // session, we have to tell that about our updated context by storing it in the session via RequestAttributes.
        ...
        req.setAttribute(HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY, context, RequestAttributes.SCOPE_SESSION);
    }
}
