...
def insert_advisor(cur, advisor, school_id):
    ...
    cur.execute('INSERT INTO advisor (name, email, is_primary, password, school_id) VALUES (%s, %s, true, %s, %s)',
        advisor['advisor_name'], advisor['advisor_email'], bcrypt.hashpw(password), school_id
    )
    return password
...
def send_emails(api_key, email_vars):
    ...
    resp = requests.post('https://api.mailgun.net/v3/busun.org/messages', auth=('api', api_key), data=params)
    print(resp.text)
